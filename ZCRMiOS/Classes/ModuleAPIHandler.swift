...
internal class ModuleAPIHandler : CommonAPIHandler
{
    ...
    internal func update( customView : ZCRMCustomView, completion : @escaping ( Result.Response< APIResponse > ) -> () )
    {
        if customView.upsertJSON.isEmpty
        {
            ZCRMLogger.logError(message: "\(ErrorCode.notModified) : No changes have been made on the customview to update, \( APIConstants.DETAILS ) : -")
            completion( .failure( ZCRMError.sdkError( code: ErrorCode.notModified, message: "No changes have been made on the customview to update", details : nil ) ) )
            return
        }
        setJSONRootKey(key: JSONRootKey.CUSTOM_VIEWS)
        setUrlPath(urlPath: "\( URLPathConstants.settings )/\( URLPathConstants.customViews )/\( customView.id )")
        addRequestParam(param: RequestParamKeys.module, value: module.apiName)
        do
        {
            let requestBody = try getCustomViewUpdateDetails(fromJSON: customView.upsertJSON)
            setRequestBody(requestBody: [ getJSONRootKey() : [ requestBody ] ])
        }
        catch
        {
            completion( .failure( typeCastToZCRMError( error ) ) )
    ...
    internal func markCustomViewsAsFavorite( _ ids : [ Int64 ], completion : @escaping ( Result.Response< APIResponse > ) -> () )
    {
        setJSONRootKey(key: JSONRootKey.CUSTOM_VIEWS)
        setUrlPath(urlPath: "\( URLPathConstants.settings )/\( URLPathConstants.customViews )/\( URLPathConstants.actions )/\( URLPathConstants.favourite )")
        addRequestParam(param: RequestParamKeys.module, value: module.apiName)
    ...
    internal func markCustomViewsAsUnFavorite( _ ids : [ Int64 ], completion : @escaping ( Result.Response< APIResponse > ) -> () )
    {
        setJSONRootKey(key: JSONRootKey.CUSTOM_VIEWS)
        setUrlPath(urlPath: "\( URLPathConstants.settings )/\( URLPathConstants.customViews )/\( URLPathConstants.actions )/\( URLPathConstants.favourite )")
        addRequestParam(param: RequestParamKeys.module, value: module.apiName)
    ...
    internal func changeCustomView( sortBy : String?, sortOrder : SortOrder?, forid id : Int64, completion : @escaping ( Result.Response< APIResponse > ) -> () )
    {
        setJSONRootKey(key: JSONRootKey.CUSTOM_VIEWS)
    ...
    private func getCustomViewUpdateDetails( fromJSON : [ String : Any? ] ) throws -> [ String : Any? ]
    {
        var updateDetails : [ String : Any ] = [:]
        
        for( key, value ) in fromJSON
        {
            if key == ResponseJSONKeys.sharedTo, let sharedDetails = value as? [ ZCRMCustomView.SharedDetails ]
            {
            ...
            else if key == ResponseJSONKeys.criteria && ZCRMSDKClient.shared.apiVersion < "v2.1"
            {
                ZCRMLogger.logError(message: "\(ErrorCode.invalidOperation) : Unsupported API version to update criteria - try with API v2.1 and above, \( APIConstants.DETAILS ) : -")
                throw ZCRMError.inValidError(code: ErrorCode.invalidOperation, message: "Unsupported API version to update criteria - try with API v2.1 and above", details: nil)
            }
            else
            {
    ...
    private func getSharedDetailsJSON( sharedDetails : [ ZCRMCustomView.SharedDetails ] ) throws -> [[ String : Any ]]
    {
        var sharedDetailsArray : [[ String : Any ]] = []
        
        for sharedTo in sharedDetails
        {
            var sharedToJSON : [ String : Any ] = [:]
            sharedToJSON[ ResponseJSONKeys.name ] = sharedTo.name
            sharedToJSON[ ResponseJSONKeys.id ] = sharedTo.id
            sharedToJSON[ ResponseJSONKeys.type ] = sharedTo.type.rawValue
            sharedToJSON[ ResponseJSONKeys.subordinates ] = sharedTo.subordinates
            sharedDetailsArray.append( sharedToJSON )
        }
        return sharedDetailsArray
    }
